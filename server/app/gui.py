...
def get_pdftocairo_path():
    # 当前脚本所在的路径
    script_dir = os.path.dirname(os.path.abspath(__file__))
    
    # 返回正确的路径
    if getattr(sys, 'frozen', False):
        # 在打包后的环境中，pdftocairo 存储在 _MEIPASS 目录下
        packed_path = os.path.join(sys._MEIPASS, 'pdftocairo')
        # 打包后的环境
        return packed_path
    else:
        # 在开发环境中，pdftocairo 存储在脚本目录下
        dev_path = os.path.join(script_dir, 'pdftocairo')
        # 开发环境
        return dev_path
...
def convert_pdf():
    ...
    # 构建 pdftocairo 命令
    command = [get_pdftocairo_path(), f"-{output_format}"]
    if scale_to:
...
def preview_first_page():
    ...
    command = [get_pdftocairo_path(), "-singlefile", "-png", pdf_file, temp_png_file]
...
convert_button.grid(row=10, column=1, sticky=tk.W)
